      ...
      <section className="container" style={{paddingTop: "0"}}>
        <div className="flex flex-col items-center text-center gap-2">
          ...
          <p className="text-slate-500 font-semibold lg:text-xl max-w-2xl">
            From sign-up to getting paid - we&#39;ve made the entire process as
            simple and fast as possible so you can start earning today.
          </p>
        </div>
          ...
          >
            {HOW_IT_WORKS_STEPS.map((step, index) => (
              <div key={index} className="relative">
                <div
                  className={`card flex flex-col gap-10 h-80 border-black text-center items-center justify-center shadow-md rounded-lg hover:shadow-medium transition-all duration-300 hover:-translate-y-1 border-0 bg-card/70 backdrop-blur-sm`}
                >
                  {/* Step number */}
                  <div
                    className="absolute text-center -top-3 left-1/2 transform -translate-x-1/2 md:mt-20"
